...
- `tools/apply-smoke-runner-digests` applies verified pipeline receipts to `image-policy.yaml` and `task-image-policy.yaml`
...
## Digest finalization workflow

1. Run the image pipeline so it emits:
...
2. Validate the receipts without writing:

   ```bash
   tools/apply-smoke-runner-digests --check
   ```

3. Apply the verified digests to policy files:

   ```bash
   tools/apply-smoke-runner-digests
   ```

4. Review and commit the policy-file diff.

The tool intentionally does not resolve registry digests itself. It only consumes pipeline receipts that were generated by the release lane.
